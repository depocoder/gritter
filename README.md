...
All environment variables should start with "GRITTER_" prefix.

For example if you see in your "gritter/settings.py" a variable named like
`random_parameter`, you should provide the "GRITTER_RANDOM_PARAMETER" 
variable to configure the value. This behaviour can be changed by overriding `env_prefix` property
in `gritter.settings.Settings.Config`.

An example of .env file:
```bash
GRITTER_RELOAD="True"
GRITTER_PORT="8000"
GRITTER_ENVIRONMENT="dev"
```
...
```bash
docker compose up -d --wait db redis rmq
```
